...
WHERE
    1 = 1 
    AND a."oya_id" = CAST (:oya_id AS INTEGER) 
    AND a."ko_bn" = CAST (:ko_bn AS INTEGER) 
    AND UPPER (TRIM(TRAILING ' ' FROM a."orphan_info")) LIKE UPPER (CONCAT ('%', :orphan_info, '%')) 
