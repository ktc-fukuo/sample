 ...
 * @author emarfkrow
 */
public class Tb6Reborn implements IEntity {
     ...
     * @return 転生
     */
    public static Tb6Reborn get(final Object param1) {
        ...
        return Queries.get(sql, map, Tb6Reborn.class);
    }
    ...
    public int insert(final LocalDateTime now, final String execId) {
        ...
        // 転生明細の登録
        if (this.tb6RebornDets != null) {
            for (Tb6RebornDet tb6RebornDet : this.tb6RebornDets) {
                if (tb6RebornDet != null) {
                    tb6RebornDet.setRebornId(this.getRebornId());
                }
                tb6RebornDet.insert(now, execId);
            }
        }

        // 転生の登録
        String sql = "INSERT INTO TB6_REBORN(\r\n      " + names() + "\r\n) VALUES (\r\n      " + values() + "\r\n)";
    ...
    private void numbering() {
        ...
        String sql = "SELECT CASE WHEN MAX(e.`REBORN_ID`) IS NULL THEN 0 ELSE MAX(e.`REBORN_ID`) * 1 END + 1 AS `REBORN_ID` FROM TB6_REBORN e";
        Map<String, Object> map = new HashMap<String, Object>();
    ...
    public int update(final LocalDateTime now, final String execId) {
        ...
        if (this.tb6RebornDets != null) {
            for (Tb6RebornDet tb6RebornDet : this.tb6RebornDets) {
                ...
                tb6RebornDet.setRebornId(this.rebornId);
                try {
                    tb6RebornDet.insert(now, execId);
                } catch (Exception e) {
                    tb6RebornDet.update(now, execId);
                }
            }
        }

        // 転生の登録
        String sql = "UPDATE TB6_REBORN\r\nSET\r\n      " + getSet() + "\r\nWHERE\r\n    " + getWhere();
        return Queries.regist(sql, toMap(now, execId));
    }
    ...
    public int delete() {
        ...
        // 転生の削除
        String sql = "DELETE FROM TB6_REBORN WHERE " + getWhere();
    ...
    /** 転生明細のリスト */
    private List<Tb6RebornDet> tb6RebornDets;

    /** @return 転生明細のリスト */
    @com.fasterxml.jackson.annotation.JsonProperty(value = "Tb6RebornDets", index = 11)
    public List<Tb6RebornDet> getTb6RebornDets() {
        return this.tb6RebornDets;
    }

    /** @param list 転生明細のリスト */
    public void setTb6RebornDets(final List<Tb6RebornDet> list) {
        this.tb6RebornDets = list;
    }

    /** @param tb6RebornDet */
    public void addTb6RebornDets(final Tb6RebornDet tb6RebornDet) {
        if (this.tb6RebornDets == null) {
            this.tb6RebornDets = new ArrayList<Tb6RebornDet>();
        }
        this.tb6RebornDets.add(tb6RebornDet);
    }

    /** @return 転生明細のリスト */
    public List<Tb6RebornDet> referTb6RebornDets() {
        this.tb6RebornDets = Tb6Reborn.referTb6RebornDets(this.rebornId);
        return this.tb6RebornDets;
    }

    /**
     * @param param1 rebornId
     * @return List<Tb6RebornDet>
     */
    public static List<Tb6RebornDet> referTb6RebornDets(final Integer param1) {
        List<String> whereList = new ArrayList<String>();
        whereList.add("REBORN_ID = :reborn_id");
        String sql = "SELECT ";
        sql += "`REBORN_ID`";
        ...
        sql += " FROM TB6_REBORN_DET a WHERE " + String.join(" AND ", whereList);
        sql += " ORDER BY ";
        sql += "REBORN_ID, REBORN_BN";
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("reborn_id", param1);
        List<Tb6RebornDet> list = Queries.select(sql, map, Tb6RebornDet.class, null, null);
        if (list != null) {
            return list;
        }
        return new ArrayList<Tb6RebornDet>();
    }
}
